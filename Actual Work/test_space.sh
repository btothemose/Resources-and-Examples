...
step4=$(ssh $fqnbn for db in ${cust}_${code}; do for i in {1..2}; do ssh bn60qs0${i} "bndb -e \"create database ${db};\"";done;done;)
...
step9=$(ssh $fqnbn for i in {1..2}; do ssh bn60qs0${i} "bnsctl StartCustomer ${cust} ${code}; bnscript -c ${cust} ${code} -f";done;)
